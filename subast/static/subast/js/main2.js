...
var ViewModel = function(){
	...
	this.init = function(){
		...
		for (var i = 0; i < initialDates.length; ++i ) {
			self.dateList.push( new PublishDate( initialDates[i] ) );
		};
	}
...
$('.btn-cotization').hover( function(){
	$('img', this).attr('src', '/static/subast/img/6-Pub-Req-36-white.png');
}, function(){
	$('img', this).attr('src', '/static/subast/img/6-Pub-Req-36.png');
});
...
/*$('.btn-activate').click( function(e){
	$.getJSON('http://192.168.0.14:8080/verify/ruc/' + $('#ruc-input').val(), function(result){
		if(result.valid)$('.activate-div').show();
		else $('.activate-div').hide();
	});
});*/

$('.uploaded-div input[type=file]').change( function(e){
	self = $(this).context;
    if (e.target.files && e.target.files[0]) {
        var reader = new FileReader();

        reader.onload = function (e) {
    	    $('.uploaded-img', self ).attr('src', e.target.result);
        }
